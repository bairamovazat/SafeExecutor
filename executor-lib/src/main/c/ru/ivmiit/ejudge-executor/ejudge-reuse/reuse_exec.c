...
format_exitcode(tTask *tsk, char *buf, int size, int code, int err) {
    ...
    switch (code) {
        case TASK_ERR_PIPE_FAILED:
            s = "pipe() failed";
            break;
        case TASK_ERR_COMM_PIPE_FAILED:
            s = "pipe() failed";
            break;
        case TASK_ERR_COMM_DUP_FAILED:
            s = "dup2() failed";
            break;
        case TASK_ERR_COMM_FCNTL_FAILED:
            s = "fcntl() failed";
            break;
        case TASK_ERR_FORK_FAILED:
            s = "fork() failed";
            break;
        case TASK_ERR_REDIR_OPEN_FAILED:
            s = "open() failed";
            break;
        case TASK_ERR_REDIR_DUP_FAILED:
            s = "dup2() failed";
            break;
        case TASK_ERR_REDIR_INVALID:
            s = "bad redirection";
            break;
        case TASK_ERR_CHDIR_FAILED:
            s = "chdir() failed";
            break;
        case TASK_ERR_EXECV_FAILED:
            s = "execv() failed";
            break;
        case TASK_ERR_RLIMIT_FAILED:
            s = "rlimit() failed";
            break;
        case TASK_ERR_PUTENV_FAILED:
            s = "putenv() failed";
            break;
        case TASK_ERR_LIMIT_CPU_FAILED:
            s = "rlimit() failed";
            break;
        default:
            s = "unknown";
...
parse_proc_pid_stat(int pid, struct process_info *info) {
    ...
    int r = sscanf(p,
                   " %c%d%d%d%d%d%u%lu%lu%lu%lu%lu%lu%lu%lu%ld%ld%ld%ld%llu%lu%ld%lu%lu%lu%lu%lu%lu%lu%lu%lu%lu%lu%lu%lu%d%d",
                   &info->state,
                   &info->ppid,
                   &info->pgrp,
                   &info->session,
